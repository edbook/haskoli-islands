...
def depart_html_form_node(self, node):
    ...
    self.body.append("</form>")
    self.body.append('<img class="submit_ok_icon" style="display: none;" '
                     'src="%s"></img>' % os.path.join(node["p_to_static"],
                                                      'Correct_20x20.png'))
...
class HtmlFormDirective(Directive):
    ...
    def run(self):
        ...
        new_node = HtmlForm(args=self.arguments,
...
class HtmlInputDirective(Directive):
    ...
    def run(self):
        ...
        return [HtmlInput(args=self.arguments,
                          el_type=el_type,
                          checked=checked,
                          maxlength=maxlength,
                          value=value)]
...
class HtmlTextareaDirective(Directive):
    ...
    def run(self):
        ...
        return [HtmlTextarea(args=self.arguments, rows=rows,
...
def setup(app):
    # html_form
    app.add_node(HtmlForm,
                 html=(visit_html_form_node, depart_html_form_node))
    app.add_directive("html-form", HtmlFormDirective)

    app.add_config_value('html_form_submit_button_name', 'Submit', True)

    # html_input
    app.add_node(HtmlInput,
                 html=(visit_html_input_node, depart_html_input_node))
    app.add_directive("html-input", HtmlInputDirective)

    # html_textarea
    app.add_node(HtmlTextarea,
                 html=(visit_html_textarea_node, depart_html_textarea_node))
