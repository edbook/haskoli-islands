...
.. admonition:: Sýnidæmi: Lestur skrár
   ...
   Þá verður hauslína strengurinn ``"Nafn Hæð\n"``, línur verður listinn ``["Jóna 168\n",
   "Kári 178\n"]``, og skráin verður strengurinn ``Hæð\nJóna 168\nKári 178"\n"``
...
.. code:: Python
   ...
   # Nú er L listinn ['Nafn Hæð', 'Jóna 168', 'Kári 178']

.. admonition:: Æfing: Textaskrá í Colab
   :class: aefing

   a. Búið til skrána "data.txt" á tölvunni ykkar með innihaldinu sem gefið er í
   ...
   b. Farið nú í Google Colab og opnið nýja vinnubók. Smellið á möpputáknið í
   ...
   c. Skrifið nú forrit sem les skrána og prentar hana jafnóðum út, eins og gert
...
þarf að beita fallinu `decode()` og eins og í síðasta kafla þarf að losa sig við
línuskiptatákn t.d. með ``strip()`` 

Dæmi um lestur skrár af vefnum er í `Verkefni 10
<https://cs.hi.is/t1a/verkefni/#skra-me-islenskum-orum-s5>`_ aftast í þessum
nótum, og þar er líka útskýring á utf-8 sniði. Hér er annað dæmi:

.. admonition:: Sýnidæmi: Skrá lesin af vefnum
   :class: synidaemi

   Skráin sem notuð var í sýnidæmunum hér á undan hefur verið sett á vefinn, í
   `https://cs.hi.is/t1a/data.txt <https://cs.hi.is/t1a/data.txt>`_. Hér er hún
   lesin og prentuð líkt og gert er í seinna sýnidæminu, til tilbreytingar með
   línunúmerum.

   .. code:: Python
             
      from urllib.request import urlopen
      f = urlopen("https://cs.hi.is/t1a/data.txt")
      for (nr,lína) in enumerate(f,1):
         strengur = lína.strip().decode()
         print(f"{nr}: {strengur}")
...
Í `Wikipedíugrein <https://en.wikipedia.org/wiki/Comma-separated_values>`_ um CSV-skrár eru fleiri dæmi t.d. í kaflanum `Example <https://en.wikipedia.org/wiki/Comma-separated_values#Example>`_. Þar er m.a. rætt hvernig hægt er að geyma svið sem innihalda bil með því að slá gæsalöppum utan um þau, en við förum ekki nánar út í þá sálma hér.
...
.. admonition:: Sýnidæmi: CSV-skrá lesin
   ...
   ``símaskrá.txt``. Eftirfarandi forrit mundi lesa hana inn og prenta á skjá
   með titillínu:

   .. code:: python

      import csv
      with open("símaskrá.txt") as f:
         reader = csv.reader(f, delimiter=';', skipinitialspace=True)
         print(f"{'NAFN':20}  {'SÍMI':7}  {'HEIMILI'}")
         print("–"*(20+7+22))
         for röð in reader:
...
Komma er sjálfgefið afmörkunartákn. Skrá sem er afmörkuð með tab-táknum má lesa með ``delimiter="\t"``.
...
Þessi skrá verður notuð í `Verkefni 14 <https://localhost:8000/verkefni/#dalkar-i-csv-skra-t6>`_.        
...
.. admonition:: Sýnidæmi: Slóðir
   ...
   a. Notandi sem er staddur í möppunni *Old* neðst til hægri á mynd
      :numref:`%s<moppur-toms>` getur vísað í skrána *Two.txt* á sömu mynd með
      slóðinni ``../../Data/Two.txt``
   b. Notandi sem er staddur í möppunni *Downloads* á mynd :numref:`%s<makka-skraakerfi>`
      getur vísað í skrána *listi.xlsx* hvort sem er með
      ``/Users/jonasson/Documents/listi.xlsx`` eða ``../Documents/listi.xlsx``.
   c. Mappan *Downloads* á mynd :numref:`%s<makka-skraakerfi>` hefur slóð
      ``/Users/jonasson/Downloads``.

.. admonition:: Æfing: Afstætt og algilt
   :class: aefing

   a. Finnið afstæða slóð skrárinnar *Format* ef notandi er staddur í
      rótarmöppunni *Tom* á mynd :numref:`%s<moppur-toms>`
         
   b. Ákvarðið algilda slóð möppunnar *Desktop* hjá notandanum *uppsetning* á
...
.. admonition:: Æfing: Ítrað yfir möppu
   ...
   a. Finnið út algilda slóð sjálfgefnu möppunnar í Colab með því að nota Path.
      
   b. Skrifið forritsbút sem ítrar yfir innihald þessarar möppu og ákvarðar
      fyrir hvern hlut sem finnst hvort hann sé skrá eða mappa.
      
   c. Búið til nýja möppu "t1a" og skrifið svo skrá þar, "tilraun.txt" með
