...
innbyggðar varpanir) er notað **associative array**, C++ og Java nota **map**,
og Python og ýmis fleiri mál nota **dictionary**. Hér notum við orðið
...
import heldur eru þær innbyggðar í málið sjálft. Lyklar vörpunar mega vera hvaða
óbreytanlegu tagi sem er: m.a. heiltölur, kommutölur, strengir og samstæður
(tuples). Þeir þurfa samt að vera óbreytanlegir, og það útilokar lista og , en
þó hvorki listar né aðrar uppflettitöflur. Til að búa til uppflettitöflu má nota
ritháttinn:
...
.. admonition:: Æfing: Uppfletting í uppflettitöflu
   ...
   a. Að nota tvær uppflettitöflur: kennitala :math:`\to` nafn og kennitala :math:`\to` skuld
   b. Að nota eina töflu: kennitala :math:`\to` (nafn, skuld)
      
   [seinni möguleikinn varpar kennitölu í par með nafni og skuld]. Það á að búa
   til töflurnar eða töfluna og skrifa svo fall ``fletta(kt,...)`` sem flettir
   upp á kennitölu og skrifar út nafn og skuld. Byrjið á að teikna töflurnar, og
   forritið gjarna báða möguleikana
