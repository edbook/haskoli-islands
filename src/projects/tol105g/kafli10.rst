...
Vigur gefið gildi
-----------------
NumPy er viðbót við Python, og öfugt við grunntögin öll gefa kost á að búa til
fasta af hverju tagi (sjá kafla :numref:`grunntög` og :numref:`söfn-inngangur`) er ekki boðið upp á vigur-fasta, heldur þarf að nota NumPy-föll til að búa til vigra.

Eitt þeirra er fallið ``np.array`` sem breytir venjulegum Python-lista af tölum í vigur. Til að búa til vigurinn :math:`x = (1,2,3,4,6)` má nota skipunina:
...
.. admonition:: Æfing: Vigratilbúningur
   ...
   - Búið til :math:`(2.0, 2.1, 2.2,\ldots, 3.9, 4.0)` með linspace
...
np.zeros((m,n),int)`` gefur heiltölu-núllfylki.

.. admonition:: Æfing: Margföldunartafla
   :class: aefing
           
   a. Búið til :math:`2 \times 2` núllfylki og prentið það út
   b. Búið til fylkin
      ...
      :math:`C`-ið.
   c. Búið til :math:`10 \times 10` fylki :math:`M` með margföldunartöflu með
      því að byrja með núllfylki og reikna svo:
...
.. admonition:: Æfing: 
   ...
   Byltið fylkinu C í síðustu æfingu og afpakkið svo dálkum þess í tvo vigra.
...
.. admonition:: Æfing: loadtxt
   ...
   a. Lesið skrána `https://cs.hi.is/t1a/hiti-urkoma.txt
      <https://cs.hi.is/t1a/hiti-urkoma.txt>`_ og plottið svo ártal og úrkomu.
   b. Lesið `https://cs.hi.is/t1a/malmar.txt <https://cs.hi.is/t1a/malmar.txt>`_ og
...
.. admonition:: Sýnidæmi: Lestur textadálka
   ...
   Skráin `https://cs.hi.is/t1a/malmar.txt <https://cs.hi.is/t1a/malmar.txt>`_ er
   með streng í fyrsta dálki, kommutölu í öðrum dálki og heiltölu í þeim þriðja.
   Hún er líka með titillínu. Hér er forrit sem les skrána

   .. code:: python

      skrá = "https://cs.hi.is/t1a/malmar.txt"
...
a. Að byrja að vista þær sem csv-skrár og lesa inn með NumPy
b. Að nota Pandas (verður vonandi á dagskrá síðar í Tölvunarfræði 1a)

Lítum hér á fyrri möguleikann. Í skránni `https://cs.hi.is/t1a/allir-malmar.xlsx <https://cs.hi.is/t1a/allir-malmar.xlsx>`_ eru upplýsingar um alla 70 málmana sem hafa sætistölu minni en 94 (plúton), nánar tiltekið (dæmi í svigum):
...
Í Excel er hægt að vista skrána sem csv-skrá með því að velja *File – Save As...*. Það hefur þegar verið gert og niðurstaðan skrifuð í 
  ...
  `https://cs.hi.is/t1a/allir-malmar.csv <https://cs.hi.is/t1a/allir-malmar.csv>`_
...
Þetta var gert með tölvuna stillta á íslensku og þá koma kommur í stað punkta í eðlisþyngdirnar. CSV-skrána má lesa inn með:
   ...
   .. code:: python
      ...
      skrá = "https://cs.hi.is/t1a/allir-malmar.csv"
...
.. admonition:: Æfing: Allir málmar
   ...
   a. Smellið á báðar skrárnar og skoðið þær
   b. Lesið csv-skrána og búið til punktarit af sætistölu og eðlisþyngd.
   c. Búið til súlurit af bræðslumarki
   d. Opnið Excel-skrána og búið til csv-skrá
