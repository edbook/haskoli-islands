...
.. admonition:: Æfing: Pókerhendur
   ...
   a. Búið í framhaldi til forrit sem dregur fimm spil, sem er einmitt fjöldi
   ...
   b. [Vinnist í samstarfi við kennara.] Búið til fall sem kannar hvort pókerhönd
...
-----------------------------------------
Til að prenta slembivigra og -fylki er hentugt að geta takmarkað fjölda aukastafa.
Það er hægt með fallinu ``np.set_printoptions``. Þetta fall hefur ýmsa stika en
hér látum við duga að segja frá þremur: ``precision = n`` og ``floatmode = "fixed"``
gefur alltaf *n* aukastafi (án floatmode koma ≤ n stafir), og ``suppress = True``
þýðir að engar tölur eru prentaðar með tugveldistáknun (t.d. -6.33e08 eða 5.22e-12).
Til að vigurinn sem kemur við sögu í Verkefni 31 prentist líkt og þar er sýnt má nota:
...
.. admonition:: Æfing: 
   ...
   a. :math:`4 \times 4` fylki með tölum úr stöðluðu normaldreifingunni, N(0,1)
   b. vigur með 7 tölum í :math:`\{2,4,6,\ldots,100\}`
   c. :math:`3 \times 3` fylki með tölum á bilinu :math:`[2,5]` með ``rng.random``
   d. :math:`3 \times 3` fylki með tölum á bilinu :math:`[2,5]` með ``rng.uniform``

   Í b-lið eru fyrst búnar til tölur í :math:`\{1,2,\ldots,50\}` og niðurstaðan
   margfölduð með 2, og í c-lið er fyrst búið til fylki með tölum í
...
------------------------
Í æfingunni "Pókerhendur" í kafla :numref:`random-einingin` og framhaldi hennar í Verkefni 33 sáum gagnsemi þess að geta valið af handahófi úr einhverju safni nokkur stök samtímis. Það er hægt með fallinu ``sample`` í random-einingunni og NumPy býður líka upp á þennan möguleika með fallinu ``rng.choice``. Bæði random og NumPy eru líka með föll sem bæði heita ``shuffle`` til að umraða lista/vigri í handahófsröð, líkt og þegar spil eru stokkuð (*shuffled*). Þriðja aðgerðin sem stundum þarf er að skila tölunum :math:`0` til :math:`n-1` í handahófsröð. Í NumPy er sérstakt fall til þess, ``permutation``, en með random þarf að nota ``sample``. 
Hér er yfirlit yfir þessi föll miðað við að L sé listi og x sé vigur:
...
.. admonition:: Æfing: Slembival og sembiröð
   ...
   a. Raðið listanum ``["Ari", "Ása", "Jói", "Óli", "Una"]`` í handahófsröð.
   b. Veljið þrjá einstaklinga af handahófi úr listanum í a-lið
   c. Lát :math:`x_k = k + k^2, k = 1,\ldots 5`. Búið til vigur úr þessum
      x-um með yfirgripi (*comprehension*) og raðið honum svo í slembiröð.
   d. Gerið ráð fyrir að ``spurning`` sé listi af spurningum og ``svar`` sé
...
jöfnur bestu línu og bestu parabólu, sem unnið er með í verkefni 24, og
auk þess jöfnu besta plans. Lesendur ættu að byrja á að skoða verkefni 24
...
hlið, en á það var lauslega minnst í verkefni 26. Hágildispunkturinn sem
prentaður er út fæst auðveldlega með því að diffra jöfnu parabólunnar og setja
sama sem núll.
...
.. admonition:: Æfing: Hæð körfuboltamanna
   ...
   `https://cs.hi.is/t1a/karfa.txt <https://cs.hi.is/t1a/karfa.txt>`_ sem fall af aldri og þyngd (skráin er með þrjá
